NAME
    DBDx::MSSQLReporter - An module to connect Perl to MS SQL Server and MS
    Data Engine
...
SYNOPSIS
    ...
    Engine, you should not even have to worry about the DSN.
            ...
            #!perl -w
            use strict;
            use DBDx::MSSQLReporter;
        
            my($connect) = "dbi:ODBC(RaiseError=>1, PrintError=>1, Taint=>1):DSN=LocalServer";
            my($reporter) = DBDx::MSSQLReporter -> new(connexion => $connect);

            print "User tables: \n";
            print join("\n", @{$reporter -> get_tableNames()}), "\n\n";

DESCRIPTION
    `DBDx::MSSQLReporter' encapsulates the connection between Perl and MS SQL
    Server.

    `DBDx::MSSQLReporter' was written so that I could teach myself about MS
...
INSTALLATION
    You install `DBDx::MSSQLReporter', as you would install any perl module,
...
AUTHOR
    `DBDx::MSSQLReporter' was written by Ron Savage *<ron@savage.net.au>* in
    2000.
